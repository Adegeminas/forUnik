...
module.exports = function proceedRequest(houses, request, callback) {
  ...
  let fileBody = 'Город,тип_улицы,название_улицы,номер_дома;отчетный_период;экономия_в_гигах \r\n \r\n';

  fileBody += response.reports.reduce((acc, house) => {
    return acc + house.reduce((acc2, period) => {
      return acc2 + period.house.address + ';' + period.purposeMonth.month + ';' + period.gigsEconomy + '\r\n';
    }, '') + '\r\n';
  }, '');
