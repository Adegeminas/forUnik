...
module.exports = function proceedRequest(houses, request, callback) {
  ...
  let fileBody = 'Город,тип_улицы,название_улицы,номер_дома;' +
    'отчетный_период;' +
    ...
    'котрольные_показания (Формат "01-2016:15,01-2017:14;");' + '\r\n \r\n';
  ...
  fs.writeFile('public/reports/' + fileName, fileBody, (err) => {
    if (err) {
      console.log(err);
      callback(response);
      return;
...
function getResultforHouse(house, request) {
  ...
  request.periods.forEach((purposePeriod) => {
    ...
    purposeAmount = (house.sameCounter)
      ...
      : (request.method === 'РЦ')
        ? (purposeMonth.O || purposeMonth.P) || purposeMonth.Q || (request.useR === 'true' && purposeMonth.R)
        : purposeMonth.Q || (purposeMonth.O || purposeMonth.P) || (request.useR === 'true' && purposeMonth.R);
