  ...
  return (
    <div>
      ...
      <legend>Результаты энергосбережения:</legend>
        ...
        <h4>Экономия составила: { result.gigsEconomy.toFixed(2)} Гкал / { result.rublesEconomy.toFixed(2) } рублей / { result.procentEconomy.toFixed(2) } процентов</h4>
        <h4>Экономия на квадратный метр: { result.economyPerMeter.toFixed(2) } рублей</h4>
        <h4>Итого в счет оплаты энергосбережения: { result.profit.toFixed(2) } рублей или { result.profitPerMeter.toFixed(2) } рублей/кв.м</h4>
...
class ReportViewer extends Component {
  ...
  render() {
    ...
    const reportView = resultToHTML(report);
